...
export default function PostList({ initialPosts }: PostListProps) {
  ...
  const formatTime = (post: Post) => {
    if (post.postedAt) return `Posted ${format(new Date(post.postedAt), "PPp")}`;
    if (post.scheduledAt) return `Scheduled ${format(new Date(post.scheduledAt), "PPp")}`;
              ...
              <div className="absolute top-2 right-2 z-10 flex items-center gap-1">
                ...
                {canUpload && (
                  <button
                    ...
                    title={post.status === "failed" ? "Retry posting to X" : "Post to X now"}
              ...
              <div className="flex items-center gap-2">
                ...
                {detailPost.impressionCount != null && (
                  <span className="flex items-center gap-1 text-xs text-gray-400">
                    ...
                    {detailPost.impressionCount.toLocaleString()} impressions
                  </span>
                )}
              </div>
              <button
              ...
              <div className="flex items-center gap-3 pt-1 flex-wrap">
                ...
                {detailPost.source !== "x" && detailPost.tweetId && (
                  ...
                  >
                    ...
                    {syncingId === detailPost.id ? "Syncing…" : "Sync from X"}
                  </button>
                )}
                {detailPost.source !== "x" && (detailPost.status === "scheduled" || detailPost.status === "failed") && (
                  ...
                  >
                    ...
                    {detailPost.status === "failed" ? "Retry" : "Post Now"}
                  </button>
                )}
              </div>
            </div>
