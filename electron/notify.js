// Native OS notification bridge for the xPilot desktop shell.
//
// The web app calls window.xpilotDesktop.notify(...) (see preload.js), which
// invokes the "xpilot:notify" IPC channel registered here. Kept as its own
// module so the handler can be exercised by a standalone harness using the same
// code path as production.
const { ipcMain, Notification } = require("electron");

// Registers the "xpilot:notify" IPC handler.
...
function registerNotificationIpc({ iconPath, onClick } = {}) {
  ipcMain.handle("xpilot:notify", (_event, options) => {
    if (!Notification.isSupported()) return { ok: false, reason: "unsupported" };
    const opts = options || {};
    const notification = new Notification({
      title: typeof opts.title === "string" ? opts.title : "xPilot",
