...
import { DEMO_TEMPLATES, type DemoTemplate } from "@/lib/sentiment-monitor-templates";
...
export default function MonitoringPage() {
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      ...
      if (data.xAccounts) {
        ...
        const defaultAcc = data.xAccounts.find((a: XAccount) => a.isDefault);
  ...
  async function runAnalysis() {
    ...
    setError("");
    try {
      const res = await fetch(`/api/monitoring/${selectedTopic.id}/analyze`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          <>
            ...
            {showCreate && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-5 space-y-4">
                ...
                <button
                  onClick={createTopic}
                  disabled={creating || !newName.trim() || newKeywords.length === 0}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 text-xs text-gray-400 flex items-center justify-between">
                        <span>
                          {t("snapshotCount", { count: topic._count.snapshots })}
                        </span>
                        <span>
                          {lastAnalyzed
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {DEMO_TEMPLATES.filter((tpl) => tpl.category === "election").map(
                    (tpl) => (
                      <TemplateCard
                        key={tpl.name}
                        tpl={tpl}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {DEMO_TEMPLATES.filter((tpl) => tpl.category === "finance").map(
                    (tpl) => (
                      <TemplateCard
                        key={tpl.name}
                        tpl={tpl}
          ...
          <>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-5">
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {(JSON.parse(selectedTopic.keywords) as string[]).map((kw) => (
                  <span
                    key={kw}
                      ...
                      <LineChart
                        data={[...selectedTopic.snapshots]
                          ...
                          .map((s) => ({
                            ...
                            [isZh ? "正面" : "Positive"]: s.positiveCount,
                            [isZh ? "负面" : "Negative"]: s.negativeCount,
                            [isZh ? "中立" : "Neutral"]: s.neutralCount,
                          }))}
                      >
