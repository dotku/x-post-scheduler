      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow mb-8">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("addSourceDesc")}</p>
          </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <a href={source.url} target="_blank" rel="noopener noreferrer"
                        className="text-sm text-blue-600 hover:underline truncate block">{source.url}</a>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        ...
                        {source._count?.images ?? source.images?.length ?? 0} {t("images")}
                      </p>
                      {source.images && source.images.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {source.images.slice(0, 4).map((img) => (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img key={img.id} src={img.blobUrl} alt={img.altText || source.name}
                    ...
                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => setExpandedId(expandedId === source.id ? null : source.id)}
