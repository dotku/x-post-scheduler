...
//
// Inside the xPilot Electron desktop shell it routes to the native OS
...
// first use. Use this for events like "post published", "publish failed",
// "AI generation done", "account logged out".
//
...
export async function notifyUser(options: NotifyOptions): Promise<boolean> {
  ...
  const desktop = window.xpilotDesktop;
